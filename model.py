...
class Model:
    ...
    def get_bbox(self) -> (Vec3f, Vec3f):
        min_vertex = max_vertex = self.verts[0].array
        for vertex in self.verts[1:]:
            min_vertex = np.minimum(min_vertex, vertex.array)
            max_vertex = np.maximum(max_vertex, vertex.array)
        return Vec3f(*min_vertex), Vec3f(*max_vertex)
    ...
    def compute_normal(self, face):
        v0 = self.point(face.array[0])
        v1 = self.point(face.array[1])
        v2 = self.point(face.array[2])
        edge1 = v1 - v0
        edge2 = v2 - v0
        N = Vec3f.cross(edge1, edge2).normalize()
        return N
    ...
    def rotate_x(self, angle_degrees):
        ...
        for i in range(self.nverts()):
            ...
            vertex_array = np.array([translated_vertex.array[0], translated_vertex.array[1], translated_vertex.array[2]]
)
