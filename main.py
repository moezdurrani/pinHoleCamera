...
def reflect(I, N):
    return -I + N * 2.0 * (Vec3f.dot(I, N))

def refract(I, N, refractive_index):
    cosi = -max(-1.0, min(1.0, (Vec3f.dot(I, N)) ))
    etai = 1.0
...
def scene_intersect(orig, dir, duck):
...
def cast_ray(orig, dir, lights, duck, depth=0):

    #print('4, cast ray')

    point, N, material = scene_intersect(orig, dir, duck)

    if depth > 4 or point is None:
        u = 0.5 + math.atan2(dir.array[0], dir.array[2]) / (2 * math.pi)
        v = 0.5 + math.asin(dir.array[1]) / math.pi
    ...
    reflect_orig = point - N * 1e-3 if (Vec3f.dot(reflect_dir, N)) < 0 else point + N * 1e-3
    refract_orig = refract_orig = point - N * 1e-3 if (Vec3f.dot(refract_dir, N)) < 0 else point + N * 1e-3

    reflect_color = cast_ray(reflect_orig, reflect_dir, lights, duck, depth + 1)
    refract_color = cast_ray(refract_orig, refract_dir, lights, duck, depth + 1)
    ...
    for light in lights:
        ...
        shadow_orig = point - N * 1e-3 if (Vec3f.dot(light_dir, N)) < 0 else point + N * 1e-3  # checking if the point lies in the shadow of lights[i]

        shadow_pt, shadow_N, tmpmaterial = scene_intersect(shadow_orig, light_dir, duck)

        if shadow_pt is not None and (shadow_pt - shadow_orig).length() < light_distance:
            continue

        diffuse_light_intensity += light.intensity * max(0.0, Vec3f.dot(light_dir, N))
        #specular_light_intensity += pow(max(0.0, -reflect(-light_dir, N).dot(dir)), material.specular_exponent) * light.intensity
        specular_light_intensity += pow(max(0.0, Vec3f.dot(reflect(-light_dir, N), dir)),material.specular_exponent) * light.intensity
...
def render(lights, duck):
    ...
    for j in range(height):
        for i in range(width):
            ...
            framebuffer[i + j * width] = cast_ray(Vec3f(0, 0, 0), dir, lights, duck)
    ...
    for j in range(height):
        for i in range(width):
            ...
            r, g, b = int(255 * max(0, min(1, pixel_color.array[0]))), int(255 * max(0, min(1, pixel_color.array[1]))), int(
                255 * max(0, min(1, pixel_color.array[2]))
            )
...
def load_environment_map(filename):
    ...
    try:
        ...
        print("Environment Map Loading time: ")
        print(envMapTime)
...
def main():
    ...
    duck = Model('objFiles/pinHole.obj', red_rubber, 0, 0, -18)
